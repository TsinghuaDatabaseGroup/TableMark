...
def process_data(name, data_path: str = None):
    ...
    data_path_is_none = data_path is None
    ...
    if data_path_is_none:
        num_train = int(num_data * 0.9)
    else:
        num_train = int(num_data * 8 / 9)
    num_test = num_data - num_train
    train_df, test_df, seed, train_idx, test_idx = train_val_test_split(data_df, cat_columns, num_train, num_test)
    ...
    # data_df.rename(columns = idx_name_mapping)
    ...
    X_num = data_df[num_columns].to_numpy().astype(np.float32)
    X_cat = data_df[cat_columns].to_numpy()
    ...
    save_dir = os.path.abspath(os.path.dirname(data_path)) if data_path is not None else f'data/{name}'
    ...
    if data_path is None:
        if os.path.exists(f'synthetic/{name}'):
...
if __name__ == "__main__":
    ...
    if args.dataname:
        ...
        for name in ['default', 'shoppers', 'beijing', 'phishing']:    
            process_data(name)
