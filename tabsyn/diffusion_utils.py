...
def sample(net, num_samples, latent_dim, class_labels=None, num_steps=50, device='cuda'):
    if class_labels is not None:
        assert num_samples == class_labels.shape[0]
    ...
    with torch.no_grad():
        for i, (t_cur, t_next) in tqdm(enumerate(zip(t_steps[:-1], t_steps[1:])), total=len(t_steps[:-1])):
            x_next = sample_step(net, num_steps, i, t_cur, t_next, x_next, class_labels)

    return x_next


def sample_step(net, num_steps, i, t_cur, t_next, x_next, class_labels=None):
    x_cur = x_next
...
class EDMLoss:
    ...
    def __call__(self, denoise_fn, x: torch.Tensor, class_labels: torch.Tensor):
        ...
        D_yn = denoise_fn(y + n, sigma, class_labels)
