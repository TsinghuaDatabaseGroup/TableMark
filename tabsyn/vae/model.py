...
class VAE(nn.Module):
    def __init__(self, d_numerical, categories, num_layers, token_dim, n_head=1, factor=32, bias=True):
        super(VAE, self).__init__()
        self.d_numerical = d_numerical
        self.categories = categories
        self.token_dim = token_dim
        self.n_head = n_head
        self.tokenizer = Tokenizer(d_numerical, categories, token_dim, bias=bias)
        self.encoder_mu = Transformer(num_layers, token_dim, n_head, token_dim, factor)
        self.encoder_logvar = Transformer(num_layers, token_dim, n_head, token_dim, factor)
        self.decoder = Transformer(num_layers, token_dim, n_head, token_dim, factor)
        self.reconstructor = Reconstructor(d_numerical, categories, token_dim)
    ...
    def decode(self, z: torch.Tensor) -> torch.Tensor:
        ...
        x_num_hat, x_cat_hat_prob = self.reconstructor(x_hat)
        return x_num_hat, x_cat_hat_prob
    ...
    def forward(self, x_num, x_cat):
        ...
        x_num_hat, x_cat_hat_prob = self.reconstructor(x_hat)
        return x_num_hat, x_cat_hat_prob, mu_z, std_z


def create_encoder_decoder(latent_transform: str, vae_path: str, device: str, **kwargs):
    vae = VAE(kwargs['d_numerical'], kwargs['categories'], kwargs['vae_num_layers'], kwargs['token_dim'], kwargs['num_vae_heads'], kwargs['vae_factor'], kwargs['vae_bias'])
    vae.load_state_dict(torch.load(vae_path, map_location=device), strict=True)
    vae.to(device)
    vae.eval()
    for param in vae.parameters():
        param.requires_grad = False
    assert latent_transform == 'none'
    return vae.encode, vae.decode
