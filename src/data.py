...
def normalize(
    ...
    if normalization == 'standard':
        ...
        assert seed == 0
    elif normalization == 'quantile-normal-1000-1e9-s0-minmax-1e-6-normal':
        assert False
        normalizer = sklearn.preprocessing.QuantileTransformer(
            output_distribution='normal',
            n_quantiles=max(min(X['train'].shape[0] // 30, 1000), 10),
        ...
        assert seed == 0
        X_train = X_train + (X_train.max(0) - X_train.min(0)) * 1e-6 * np.random.default_rng(seed + 1).standard_normal(X_train.shape)
    elif normalization == 'quantile-normal-1000-1e9-s0-minmax-1e-6-uniform':
        assert False
        normalizer = sklearn.preprocessing.QuantileTransformer(
            output_distribution='normal',
            n_quantiles=max(min(X['train'].shape[0] // 30, 1000), 10),
        ...
        assert seed == 0
        X_train = X_train + (X_train.max(0) - X_train.min(0)) * 1e-6 * np.random.default_rng(seed + 1).uniform(low=-1, high=1, size=X_train.shape)
    elif normalization == 'robust':
        assert False
        normalizer = sklearn.preprocessing.RobustScaler(
            with_centering=True,     # 是否对中位数居中，推荐 True
            with_scaling=True,       # 是否按 IQR 缩放，推荐 True
            quantile_range=(25.0, 75.0),
        )
    elif normalization == 'power':
        assert False
        normalizer = sklearn.preprocessing.PowerTransformer(
            method='yeo-johnson',
            standardize=True,
            copy=True,
        )
